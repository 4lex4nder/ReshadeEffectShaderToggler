...
void ResourceManager::DisposeGroupBuffers(reshade::api::effect_runtime* runtime)
{
    runtime->get_command_queue()->wait_idle();
...
void ResourceManager::DisposeGroupBuffer(reshade::api::effect_runtime* runtime, const ShaderToggler::ToggleGroup* group)
{
    const auto& buf = group_buffers.find(group);
    if (buf != group_buffers.end())
    {
        auto& [buf_res, buf_rtv, buf_rtv_srgb, buf_srv] = buf->second;
...
void ResourceManager::CheckGroupBuffer(reshade::api::command_list* cmd_list, reshade::api::device* device, reshade::api::effect_runtime* runtime, const ShaderToggler::ToggleGroup* group)
{
    ...
    auto& [buf_res, buf_rtv, buf_rtv_srgb, buf_srv] = group_buffers[group];

    DisposeGroupBuffer(runtime, group);
...
void ResourceManager::SetGroupBufferHandles(const ShaderToggler::ToggleGroup* group, reshade::api::resource* res, reshade::api::resource_view* rtv, reshade::api::resource_view* rtv_srgb, reshade::api::resource_view* srv)
{
    const auto& buf = group_buffers.find(group);
    if (buf != group_buffers.end())
    {
        const auto& [buf_res, buf_rtv, buf_rtv_srgb, buf_srv] = buf->second;
...
bool ResourceManager::IsCompatibleWithGroupFormat(reshade::api::effect_runtime* runtime, reshade::api::resource res, const ShaderToggler::ToggleGroup* group)
{
    const auto& buf = group_buffers.find(group);
    if (buf != group_buffers.end())
    {
